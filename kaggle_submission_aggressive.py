"""
AGGRESSIVE Ensemble Submission - Maximum sensitivity
Uses very aggressive allocation mapping to break conservative predictions
"""
import os
import sys
...
class AggressiveEnsemblePredictor:
    """Aggressive ensemble predictor with maximum sensitivity"""
    ...
    def predict_allocation(self, test_row_pl: pl.DataFrame) -> float:
        """Main prediction function with aggressive allocation"""
        # Convert to pandas
        ...
        try:
            ...
            # ULTRA-AGGRESSIVE allocation mapping
            # Scale factor 0.0005 = 0.05% predicted return → full swing
            scale_factor = 0.0005  # Ultra sensitive!
            ...
            if len(self.test_rows) <= 10:  # More logging
                ...
                print(f"  Ensemble: {ensemble_prediction:.8f}")
...
def predict(test: pl.DataFrame) -> float:
    ...
    if predictor is None:
        print("=" * 70)
        print("INITIALIZING AGGRESSIVE ENSEMBLE PREDICTOR")
        print("=" * 70)
        predictor = AggressiveEnsemblePredictor()
        print("=" * 70)
