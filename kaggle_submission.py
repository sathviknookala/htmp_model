"""
Simple Linear Strategy: Training-data history + linear mapping (scale 0.01)
Reverting to earlier best-performing approach for final attempt
"""
import os
import sys
...
class HullTacticalPredictor:
    ...
    def predict_allocation(self, test_row_pl: pl.DataFrame) -> float:
        ...
        try:
            ...
            predicted_return = float(self.gbm_model.predict(X)[0])

            # Simple linear mapping to allocation in [0, 2]
            # Scale factor 0.01 → 1% predicted excess return maps to full swing
            scale_factor = 0.01
            if predicted_return > 0:
                allocation = 1.0 + min(predicted_return / scale_factor, 1.0)
            else:
                allocation = 1.0 + max(predicted_return / scale_factor, -1.0)
