"""
Volatility-Weighted Strategy: Scale allocation by prediction strength and recent volatility
For better Sharpe ratio through risk management
"""
import os
import sys
...
class HullTacticalPredictor:
    ...
    def predict_allocation(self, test_row_pl: pl.DataFrame) -> float:
        ...
        try:
            ...
            # Track predictions for volatility estimation
            self.recent_predictions.append(predicted_return)
            if len(self.recent_predictions) > 20:
                self.recent_predictions = self.recent_predictions[-20:]
            
            # VOLATILITY-WEIGHTED STRATEGY
            # Scale by prediction magnitude relative to recent volatility
            if len(self.recent_predictions) >= 5:
                pred_std = np.std(self.recent_predictions)
                if pred_std > 0:
                    # Normalize by volatility
                    z_score = predicted_return / (pred_std + 1e-8)
                    # Scale allocation by z-score strength
                    allocation = 1.0 + np.tanh(z_score * 2) * 1.0  # tanh keeps in [-1, 1]
                else:
                    # Fallback if no volatility
                    allocation = 1.0 + np.sign(predicted_return) * min(abs(predicted_return) / 0.002, 1.0)
            else:
                # Cold start: use aggressive scaling
                allocation = 1.0 + np.sign(predicted_return) * min(abs(predicted_return) / 0.002, 1.0)
